...
## Offering 추출 클라이언트: 공식 벤더 SDK + 모델 체인 에스컬레이션
- **날짜**: 2026-07-25
- **상태**: 제안 (리뷰 대기)
...
- 1차: `gpt-5-mini` (실측 근거). 상위: [미정 — 품질 우선 티어]
...
### 결정 2 — 연동 방식: 공식 벤더 SDK, 벤더별 어댑터

| 방식 | 스키마 강제 보장 | 추상화 | 판정 |
|---|---|---|---|
| Spring AI ChatClient | `.entity()`가 네이티브 strict json_schema 를 쓰는지 버전 의존 — 와이어 불투명 | 프레임워크 (포트 위에 한 겹 더) | 기각 |
| **공식 벤더 SDK** (`com.openai:openai-java`) | 실험에서 검증한 요청 형태(strict json_schema) 그대로 재현 | 없음 — 포트가 유일한 추상화 | **채택** |
| 직접 HTTP | 가능하나 재시도·타입 예외·파싱 재구현 | 없음 | 기각 |

- 벤더 경계는 이미 도메인 포트(`OfferingSource`)가 담당 — Spring AI의 "프로바이더 스왑" 이점은 포트 뒤에서 이중 추상화가 됨
...
2026-02-12 결정은 감성 분석 맥락이었고 미구현 상태. 본 결정이 채택되면 LLM 연동 방식 전반을 대체(supersede)하며, 감성 분석 구현 시에도 동일 방식(공식 SDK + 포트)을 적용한다.
