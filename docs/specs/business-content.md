...
### stage 2a 실측 2차 — 재시도 배치 + 가드 개선 + 무료 재검증 (2026-08-08)

- **누적 성공 41 (EXTRACTED 33 + CORRECTED 8, 89.1%) / 실패 5 (10.9%)** — Offering 954행, 41개사. 1차 56.5% → 2차 89.1%.
- **경로**: 실패 20건 mini 재시도 배치(19분, 7건 회복) → 재실패 13건은 note·drafts 영속화 → 사유 분석 → 가드 3개소 개선 → `reverify`(LLM 재호출 0원)로 8건 추가 회복.
- **가드 개선 3개소** (모두 "LLM 답은 맞는데 가드가 억울하게 떨어뜨린" 패턴):
  - **의역 부분 근거 수용**: 제품명 전문 일치 실패 시 구두점(`(),·/~`) 세그먼트 중 하나라도 원문에 있으면 CORRECTED로 수용 (예: "인터넷전문은행(무점포 형태의 디지털 은행)" → "인터넷전문은행" 앵커).
  - **회계 음수 표기**: `(567,779)`·`△`·`▲` 를 음수 금액·비중의 근거로 인정 (농심).
  - **복수 파이 수용**: 비중 합이 115 초과라도 100의 배수 ±15 안이면 "구분 차원 결손(fiscalYear null 등)으로 합쳐진 복수 100% 파이"로 보고 CORRECTED 수용 (NAVER 400, POSCO홀딩스 연도별 200, 현대건설 300). 배수 근처가 아닌 초과(예: 132)는 여전히 실패.
- **잔여 실패 5 (KB금융·강원랜드·미래에셋증권·삼성생명·카카오뱅크)**: 가드가 아니라 추출 자체의 품질 문제 — "카지노 매출 비중", "신용카드 및 체크카드 회원 수"처럼 제품이 아닌 지표명을 제품으로 뽑은 케이스. 무료 재검증으로 못 살리는 층이며 프롬프트 개선(제품 vs 지표 구분 규칙) 후 유료 재추출 대상. 금융 서식 리스크의 실체가 "표 구조"가 아니라 "제품 개념 부재"로 좁혀짐.
- 다음(승인제): ① 프롬프트에 제품/지표 구분 규칙 추가 후 잔여 5건 재추출 ② 전수 배치 결정 (89% 성공률이면 전수 가치 충분) ③ 역방향 조회 API.

### stage 2a 실측 3차 — 프롬프트 규칙 9 검증 (2026-08-09)

- **누적 성공 44 (EXTRACTED 36 + CORRECTED 8, 95.7%) / 실패 2 (4.3%)** — Offering 1,017행, 44개사. 1차 56.5% → 2차 89.1% → 3차 95.7%.
- **프롬프트 규칙 9(제품/지표 구분) 검증 성공**: known-violation 세트 5건 중 지표명 오추출이 원인이던 KB금융·강원랜드·카카오뱅크 3건 회복. 배치는 5건인데 12시간 이상 소요 — 배치 소요 시간은 요청 수와 무관하게 분산 큼(1차 10분, 2차 19분, 3차 ~13시간). 24시간 계약 내이므로 수거 폴링은 장주기로.
- **잔여 실패 2 (미래에셋증권·삼성생명)**: 실패 층이 또 한 단계 내려감 — 이번엔 공백 단위 의역("해외주식 중개", "임대 및 빌딩운영 서비스")이라 구두점 세그먼트 매칭이 못 받음. 가드를 공백 토큰 매칭까지 풀지(전 토큰 존재 요구로 조이면 오탐 방어 가능), 2/46은 수용하고 전수로 갈지 판단 필요.
- 다음(승인제): ① 전수 배치 (95.7%면 충분) ② 잔여 2건은 가드 공백 토큰 완화 또는 수용 ③ 역방향 조회 API.
