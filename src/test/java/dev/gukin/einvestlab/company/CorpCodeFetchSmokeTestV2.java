...
public class CorpCodeFetchSmokeTestV2 {
    ...
    void shouldFetchCorpCodeWithValidDartApiKey() throws IOException, InterruptedException, XMLStreamException {
        ...
        try (InputStream body = response.body()) {
            zipBytes = body.readAllBytes();   // 3.6MB 메모리 적재. 끝.
        }

        for (int i = 1; i <= 10; i++) {
            totalCount = parseOnce(zipBytes, batch, totalCount);
            System.out.printf("--- iteration %d done, total=%d ---%n", i, totalCount);
        }

        System.out.println("최종 총 처리: " + totalCount);
    }

    private int parseOnce(byte[] zipBytes, List<CompanyRow> batch, int totalCount) throws IOException, XMLStreamException {
        try (ByteArrayInputStream byteIn = new ByteArrayInputStream(zipBytes);
             ZipInputStream zipStream = new ZipInputStream(byteIn)
        ) {
